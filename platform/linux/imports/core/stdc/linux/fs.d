...
enum IoIntegrity {
	CheckGuard = 1U << 0, /* enforce guard check */
	CheckRefTag = 1U << 1, /* enforce ref check */
	CheckAppTag = 1U << 2, /* enforce app check */

	ValidFlags = CheckGuard | CheckRefTag | CheckAppTag,
}

enum Seek {
	Set = 0, /* seek relative to beginning of file */
	Cur = 1, /* seek relative to current file position */
	End = 2, /* seek relative to end of file */
	Data = 3, /* seek to the next data */
	Hole = 4, /* seek to the next hole */
	Max = Hole,
}
...
enum LBMD_PI_CAP_INTEGRITY = 1 << 0;
enum LBMD_PI_CAP_REFTAG = 1 << 1;

/* Checksum types for Protection Information */
enum LBMD_PI_CSUM_NONE = 0;
enum LBMD_PI_CSUM_IP = 1;
enum LBMD_PI_CSUM_CRC16_T10DIF = 2;
enum LBMD_PI_CSUM_CRC64_NVME = 4;

/* sizeof first published struct */
enum LBMD_SIZE_VER0 = 16;
 ...
 */
enum FS_XFLAG {
	RealTime = 0x00000001, /* data in realtime volume */
	PreAlloc = 0x00000002, /* preallocated file extents */
	Immutable = 0x00000008, /* file cannot be modified */
	Append = 0x00000010, /* all writes append */
	Sync = 0x00000020, /* all writes synchronous */
	NoATime = 0x00000040, /* do not update access time */
	NoDump = 0x00000080, /* do not include in backups */
	RTInherit = 0x00000100, /* create with rt bit set */
	ProjInherit = 0x00000200, /* create with parents projid */
	NoSymLinks = 0x00000400, /* disallow symlink creation */
	ExtSize = 0x00000800, /* extent size allocator hint */
	ExtSzInherit = 0x00001000, /* inherit inode extent size */
	NoDefrag = 0x00002000, /* do not defragment */
	FileStream = 0x00004000, /* use filestream allocator */
	DAX = 0x00008000, /* use DAX for IO */
	COWExtSize = 0x00010000, /* CoW extent size allocator hint */
	Verity = 0x00020000, /* fs-verity enabled */
	HasAttr = 0x80000000, /* no DIFLAG for this	*/
}
...
enum BLKBSZGET = _IOR!(0x12, 112, size_t);
enum BLKBSZSET = _IOW!(0x12, 113, size_t);
enum BLKGETSIZE64 =
	_IOR!(0x12, 114, size_t); /* return device size in bytes (u64 *arg) */
// enum BLKTRACESETUP = _IOWR!(0x12, 115, blk_user_trace_setup) ;
...
enum BLKGETDISKSEQ = _IOR!(0x12, 128, __u64);

/* 130-136 and 142 are used by zoned block device ioctls (uapi/linux/blkzoned.h) */
/* 137-141 are used by blk-crypto ioctls (uapi/linux/blk-crypto.h) */
// enum BLKTRACESETUP2 = _IOWR!(0x12, 142, blk_user_trace_setup2);
...
enum FIFREEZE = _IOWR!('X', 119, int); /* Freeze */
enum FITHAW = _IOWR!('X', 120, int); /* Thaw */
enum FITRIM = _IOWR!('X', 121, fstrim_range); /* Trim */
enum FICLONE = _IOW!(0x94, 9, int);
enum FICLONERANGE = _IOW!(0x94, 13, file_clone_range);
enum FIDEDUPERANGE = _IOWR!(0x94, 54, file_dedupe_range);

enum FSLABEL_MAX = 256; /* Max chars for the interface; each fs may differ */

enum FS_IOC_GETFLAGS = _IOR!('f', 1, long);
enum FS_IOC_SETFLAGS = _IOW!('f', 2, long);
enum FS_IOC_GETVERSION = _IOR!('v', 1, long);
enum FS_IOC_SETVERSION = _IOW!('v', 2, long);
// enum FS_IOC_FIEMAP = _IOWR!('f', 11, fiemap);
enum FS_IOC32_GETFLAGS = _IOR!('f', 1, int);
enum FS_IOC32_SETFLAGS = _IOW!('f', 2, int);
enum FS_IOC32_GETVERSION = _IOR!('v', 1, int);
enum FS_IOC32_SETVERSION = _IOW!('v', 2, int);
enum FS_IOC_FSGETXATTR = _IOR!('X', 31, fsxattr);
enum FS_IOC_FSSETXATTR = _IOW!('X', 32, fsxattr);
enum FS_IOC_GETFSLABEL = _IOR!(0x94, 49, char[FSLABEL_MAX]);
enum FS_IOC_SETFSLABEL = _IOW!(0x94, 50, char[FSLABEL_MAX]);
/* Returns the external filesystem UUID, the same one blkid returns */
enum FS_IOC_GETFSUUID = _IOR!(0x15, 0, fsuuid2);
/*
 * Returns the path component under /sys/fs/ that refers to this filesystem;
 * also /sys/kernel/debug/ for filesystems with debugfs exports
 */
enum FS_IOC_GETFSSYSFSPATH = _IOR!(0x15, 1, fs_sysfs_path);
/* Get logical block metadata capability details */
enum FS_IOC_GETLBMD_CAP = _IOWR!(0x15, 2, logical_block_metadata_cap);
...
enum SyncFileRange {
	...
	Write = 2,
	WaitAfter = 4,
	WriteAndWait = Write | WaitBefore | WaitAfter,
}
...
enum RWF : int {
	...
	DontCache = 0x00000080,

	/* prevent pipe and socket writes from raising SIGPIPE */
	NoSignal = 0x00000100,

	/* mask of flags supported by the kernel */
	Supported = HiPri | DSync | Sync | NoWait | Append | NoAppend | Atomic
		| DontCache | NoSignal,
}

enum PROCFS_IOCTL_MAGIC = 'f';

/* Pagemap ioctl */
enum PAGEMAP_SCAN = _IOWR!(PROCFS_IOCTL_MAGIC, 16, pm_scan_arg);

/* Bitmasks provided in pm_scan_args masks and reported in page_region.categories. */
enum PAGE_IS_WPALLOWED = 1 << 0;
enum PAGE_IS_WRITTEN = 1 << 1;
enum PAGE_IS_FILE = 1 << 2;
enum PAGE_IS_PRESENT = 1 << 3;
enum PAGE_IS_SWAPPED = 1 << 4;
enum PAGE_IS_PFNZERO = 1 << 5;
enum PAGE_IS_HUGE = 1 << 6;
enum PAGE_IS_SOFT_DIRTY = 1 << 7;
enum PAGE_IS_GUARD = 1 << 8;
...
enum PageMapScan {
	WriteProtectMatching = 1 << 0, /* Write protect the pages matched. */
	CheckWriteProtectAsync =
...
/* /proc/<pid>/maps ioctl */
enum PROCMAP_QUERY = _IOWR!(PROCFS_IOCTL_MAGIC, 17, procmap_query);
 ...
 */
enum FS_IOC_SHUTDOWN = _IOR!('X', 125, __u32);

/*
 * Flags for FS_IOC_SHUTDOWN
 */
enum FsShutdownFlags {
	Default = 0x0,
	LogFlush = 0x1, /* flush log but not data*/
	NoLogFlush = 0x2, /* don't flush log nor data */
}
