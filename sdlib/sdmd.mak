LIBSMD_SRC = sdlib/d/rt/eh.d sdlib/d/rt/dwarf.d sdlib/sdc/finalizer.d

LIBSDMD = lib/libsdmd.a

# XXX: This kind of stunt would be better suited for the configure step,
# but we don't have one.
NATIVE_OBJECT_D = $(shell echo "int dummy;" | $(DMD) -v -o- - | sed -ne 's/^import[[:space:]]*object[[:space:]]*(\(.*\))/\1/p')
NATIVE_DRUNTIME ?= $(dir $(NATIVE_OBJECT_D))
LIBSDMD_IMPORTS = $(addprefix -I, $(LIBSDMD_PLATFORM_IMPORTS)) -I$(NATIVE_DRUNTIME)  -Isdlib

obj/sdmd.o: $(LIBSMD_SRC)
	@mkdir -p lib obj
	$(DMD) -c -of"$@" $(LIBSMD_SRC) -makedeps="$@.deps" $(ARCHFLAG) $(LIBSDMD_IMPORTS)

$(LIBSDMD): obj/sdmd.o $(LIBSDMD_PLATFORM_OBJ)
	ar rcs "$@" $^
