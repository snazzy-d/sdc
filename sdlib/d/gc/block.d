...
unittest reserve_release {
	...
	checkRangeState(5, 5, PagesInBlock - 5);

	// Second allocation.
	assert(block.reserve(5) == 5);
	checkRangeState(10, 10, PagesInBlock - 10);

	// Check that freeing the first allocation works as expected.
	block.release(0, 5);
	checkRangeState(5, 10, PagesInBlock - 10);
	...
	checkRangeState(12, 17, PagesInBlock - 17);

	// A new allocation that fits is allocated in there.
	assert(block.reserve(5) == 0);
	checkRangeState(17, 17, PagesInBlock - 17);

	// Make sure we keep track of the longest free range
	// when releasing pages.
	block.release(10, 7);
	checkRangeState(10, 17, PagesInBlock - 10);

	block.release(0, 5);
	checkRangeState(5, 17, PagesInBlock - 10);

	block.release(5, 5);
	checkRangeState(0, 17, PagesInBlock);
	...
	checkRangeState(PagesInBlock, 512, 0);

	// Release in the middle.
	block.release(100, 4);
	checkRangeState(PagesInBlock - 4, 512, 4);

	// Release just before and after.
	block.release(104, 4);
	checkRangeState(PagesInBlock - 8, 512, 8);

	block.release(96, 4);
	checkRangeState(PagesInBlock - 12, 512, 12);

	// Release futher along and then bridge.
	block.release(112, 4);
	checkRangeState(PagesInBlock - 16, 512, 12);

	block.release(108, 4);
	checkRangeState(PagesInBlock - 20, 512, 20);

	// Release first and last.
	block.release(0, 4);
	checkRangeState(PagesInBlock - 24, 512, 20);
...
unittest reserveAt {
	...
	checkRangeState(64, 64, PagesInBlock - 64);

	// Grow it by 32 pages:
	assert(block.reserveAt(64, 32));
	checkRangeState(96, 96, PagesInBlock - 96);

	// Grow it by another 32 pages:
	assert(block.reserveAt(96, 32));
	checkRangeState(128, 128, PagesInBlock - 128);

	// Second allocation:
	assert(block.reserve(256) == 128);
	checkRangeState(384, 384, PagesInBlock - 384);
	...
	checkRangeState(256, 512, PagesInBlock - 256);
	...
	checkRangeState(506, 512, PagesInBlock - 506);
...
unittest track_dirty {
	...
	checkRangeState(15, 20, PagesInBlock - 20);
}
