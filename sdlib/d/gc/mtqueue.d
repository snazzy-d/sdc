...
struct ConcurentQueue(T, size_t S) {
private:
	...
	static assert(isPow2(S), "S must be a power of 2!");
	static assert(S >= PointerInCacheLine,
	              "S must be greater than a cache line!");

	/**
	 * In order to avoid false sharing, we make sure each of the control
	 * elements of the ring buffer are on their own cache line.
	 */
	alias Padding = size_t[PointerInCacheLine - 1];
...
public:
	this(T[] buffer) {
		...
		assert(buffer.length == S, "Invalid buffer size!");

		_buffer = buffer.ptr;
	}

	void insert(T[] elements, ref Overflow!T overflow) shared {
		overflow.flush!S(this);
...
unittest mtqueue {
	...
	auto queue = shared(ConcurentQueue!(size_t, 32))(qbuf);
...
public:
	...
	void flush(size_t S)(ref shared ConcurentQueue!(T, S) queue) {
...
unittest overflow {
	...
	auto queue = shared(ConcurentQueue!(size_t, 8))(qbuf);
