...
extern(D) {
	auto ELF32_ST_BIND(T)(T val) {
		return cast(ubyte) val >> 4;
	}

	auto ELF32_ST_TYPE(T)(T val) {
		return val & 0xf;
	}

	auto ELF32_ST_INFO(B, T)(B bind, T type) {
...
extern(D) {
	auto ELF32_ST_VISIBILITY(O)(O o) {
		return o & 0x03;
	}
...
extern(D) {
	auto ELF32_R_SYM(V)(V val) {
		return val >> 8;
	}

	auto ELF32_R_TYPE(V)(V val) {
		return val & 0xff;
	}

	auto ELF32_R_INFO(S, T)(S sym, T type) {
	...
	auto ELF64_R_INFO(S, T)(S sym, T type) {
		return (sym << 32) + (type);
	}
}
...
struct Elf64_Elf32_EhdrVerdaux {
	Elf64_Word vda_name;
	Elf64_Word vda_next;
}
...
extern(D) {
	auto ELF32_M_SYM(I)(I info) {
		return info >> 8;
	}

	auto ELF32_M_SIZE(I)(I info) {
		return cast(ubyte) info;
	}

	auto ELF32_M_INFO(S, SZ)(S sym, SZ size) {
		return (sym << 8) + cast(ubye) size;
	}
}
...
extern(D) auto EF_ARM_EABI_VERSION(F)(F flags) {
