...
shared static this() {
	...
	assert(fpctrl.rounding == FloatingPointControl.roundToNearest,
	       "Floating point must round to the nearest.");
}
...
private:
	...
	enum MantissaShift = KindBits + ExponentBits;

	import std.bitmanip;
	mixin(bitfields!(
		// sdfmt off
		Kind, "_kind", KindBits,
		uint, "_exponent", ExponentBits,
	...
	this(ulong mantissa, int exponent) {
		_kind = Kind.Decimal;
		_exponent = exponent + ExponentOffset;
		_prefix = (mantissa >> 32);
		_base = mantissa & uint.max;
	}
...
public:
	...
	static recompose(uint base, uint extra) {
		auto p = PackedFloat(base, 0);

		p._kind = cast(Kind) (extra & KindMask);
		p._exponent = (extra >> ExponentShift) & ExponentMask;
		p._prefix = extra >> MantissaShift;
	...
	@property
	uint extra() const {
		return _kind | (_exponent << ExponentShift)
			| (_prefix << MantissaShift);
	}
...
struct SoftFloat {
	...
	T fromDecimalTo(T)() const if (isFloatingPoint!T) in(!hex) {
		...
		if (exponent > C.LargestPowerOfTen) {
			return T.infinity;
		}

		/**
		 * We the apporach described by Daniel Lemire in
		 * https://arxiv.org/pdf/2101.11408.pdf .
		 *
		 * Note that we are going to skip the higher precision case
		 * as it was demonstrated to be unecessary in subsequent work:
		...
		import util.math;
		auto lz = countLeadingZeros(mantissa);
		auto product = computeProductAproximation(mantissa << lz, exponent);
 ...
 *
 * We are trying to find m' and e' such as m * 10^e = m' * 2^e' .
 * Because m * 10^e = m * 2^e * 5^e the only rleevent part is m * 5^e .
 */
ulong[2] computeProductAproximation(ulong mantissa, int exponent) {
	alias C = TypeConstants!double;
	auto i = 2 * (exponent - C.SmallestPowerOfTen);

	import util.math;
	auto hi = mulhi(mantissa, POWERS_OF_5[i]);
	auto lo = mantissa * POWERS_OF_5[i];

	auto second = mulhi(mantissa, POWERS_OF_5[i + 1]);
	lo += second;
	hi += second > lo;
...
unittest {
	alias CF = TypeConstants!float;
	alias CD = TypeConstants!float;
	alias PF = PackedFloat!24;

	auto c = new Context();

	void checkDecode(PF p, string expected) {
		import core.stdc.stdlib;
		assert(p.to!float(c) == strtof(expected.ptr, null), expected);
...
unittest {
	...
	immutable TestCase[] DecimalTestVectors = [
		...
		TestCase("1e-64", 1, CF.SmallestPowerOfTen, 0x00000000,
		         0x32a50ffd44f4a73d),
		TestCase("0.7006492321624085354e-45", 7006492321624085354,
		         CF.SmallestPowerOfTen, 0x00000000, 0x3690000000000000),
		TestCase("0.7006492321624085355e-45", 7006492321624085355,
		         CF.SmallestPowerOfTen, 0x00000001, 0x3690000000000000),
		TestCase("0.3402823567797336616e39", 3402823567797336616, 20,
		         0x7f7fffff, 0x47effffff0000000),
		TestCase("0.3402823567797336617e39", 3402823567797336617, 20,
		         0x7f800000, 0x47effffff0000000),
		TestCase("3e38", 3, CF.LargestPowerOfTen, 0x7f61b1e6,
		         0x47ec363cbf21f28a),
		TestCase("4e38", 4, CF.LargestPowerOfTen, 0x7f800000,
		         0x47f2ced32a16a1b1),
		TestCase("1e39", 1, CF.LargestPowerOfTen + 1, 0x7f800000,
		         0x48078287f49c4a1d),

		// Test lower double exponent boundaries.
		TestCase("1.8446744073709551615e-324", ulong.max,
		         CD.SmallestPowerOfTen - 1, 0x00000000, 0x0000000000000000),
		TestCase("1e-342", 1, CD.SmallestPowerOfTen, 0x00000000,
		         0x0000000000000000),
		TestCase("0.2470328229206232720e-323", 2470328229206232720,
		         CD.SmallestPowerOfTen, 0x00000000, 0x0000000000000000),
		TestCase("0.2470328229206232721e-323", 2470328229206232721,
		         CD.SmallestPowerOfTen, 0x00000000, 0x0000000000000001),
		TestCase("1.7976931348623158079e308", 17976931348623158079, 289,
		         0x7f800000, 0x7fefffffffffffff),
		TestCase("1.7976931348623158080e308", 17976931348623158080, 289,
		         0x7f800000, 0x7ff0000000000000),
		TestCase("1e308", 1, CD.LargestPowerOfTen, 0x7f800000,
		         0x7fe1ccf385ebc8a0),
		TestCase("2e308", 2, CD.LargestPowerOfTen, 0x7f800000,
		         0x7ff0000000000000),
		TestCase("1e309", 1, CD.LargestPowerOfTen + 1, 0x7f800000,
		         0x7ff0000000000000),

		// Float subnormal to normal.
		TestCase("1.1754942807573642917e-38", 11754942807573642917, -57,
		         0x007fffff, 0x380fffffe0000000),
		TestCase("1.1754942807573642918e-38", 11754942807573642918, -57,
		         0x00800000, 0x380fffffe0000000),

		// Double subnormal to normal.
		TestCase("0.2225073858507201136e-307", 2225073858507201136, -326,
		         0x00000000, 0x000fffffffffffff),
		TestCase("0.2225073858507201137e-307", 2225073858507201137, -326,
		         0x00000000, 0x0010000000000000),

		// Rounds differently for floats and doubles.
		// https://lemire.me/blog/2021/11/30/can-you-safely-parse-a-double-and-then-cast-to-a-float/
		// Archive: https://archive.ph/bNxnD
		TestCase("0.004221370676532388", 4221370676532388, -18, 0x3b8a536d,
		         0x3f714a6d90000000),

		// Require precise aproximation.
		TestCase("0.2440254496e57", 2440254496, 47, 0x7f800000,
		         0x4ba3e77d385929cc),

		// Integers past the range they can be represented exactly.
		TestCase("16777217", 16777217, 0, 0x4b800000, 0x4170000010000000),
		TestCase("9007199254740993", 9007199254740993, 0, 0x5a000000,
		         0x4340000000000000),

		// Smallest exact 64 bit number.
		TestCase("7450580596923828125e-27", 7450580596923828125, -27,
...
immutable ulong[1302] POWERS_OF_5 = [
	// sdfmt off
	...
	0x84a57695fe98746d, 0x014bb630f7604b57,
