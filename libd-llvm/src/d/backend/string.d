...
final class StringGen {
	...
	auto buildDString(string str) in {
		...
		return stringLiterals.get(str, stringLiterals[str] = {
			...
			auto length = LLVMConstInt(LLVMInt64TypeInContext(pass.context), str.length, false);
			
			/*
			// skip 0 termination.
			auto indices = [LLVMConstInt(LLVMInt64TypeInContext(pass.context), 0, true), LLVMConstInt(LLVMInt64TypeInContext(pass.context), 0, true)];
			...
			return LLVMConstStructInContext(pass.context, [length, ptr].ptr, 2, false);
		}());
	}
}
