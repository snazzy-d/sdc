...
Run make.
Copy bin/sdc into the root. Compile the runner using dmd, and run it to run the tests using SDC. You'll need LLVM's `llc` in your `$PATH`, and `opt` if you want to use `--optimise`. If you build a 64 bit SDC, be aware that DMD has some bugs that make it crash in certain situations.
