...
int runUtility(U, string File = __FILE__)(auto ref U utility, string[] args) {
	...
	writefln("Summary: %s tests, %s pass%s, %s failure%s, %.2f%% pass rate, "
			 ...
			 result.count - result.passed, (result.count - result.passed) == 1 ? "" : "s",
...
Options parseArguements(string File, U)(ref U utility, ref string[] args) {
	...
	try {
		...
		if (help_info.helpWanted) {
			...
			writeln("Usage: ", File, " [options]", U.XtraDoc ? " [" ~ U.XtraDoc ~ "]" : "");
			writeln("Options:");
...
Result runTasks(R)(R tasks, ref const Options options) {
	...
	void waitForJob() {
		...
		if (fixed) {
			...
			writeln(", REGRESSION");
		} else if ((options.displayOnlyFailed && !testResult.result) || !options.displayOnlyFailed) {
			writeln();
		}
	}
...
void setcwd() {
	...
	import std.path, std.array;
	string dirname = __FILE__.asAbsolutePath().asNormalizedPath().array().dirName();
